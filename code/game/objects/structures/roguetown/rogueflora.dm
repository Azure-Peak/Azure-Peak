...
/obj/structure/flora/roguegrass/get_mechanics_examine(mob/user)
	...
	. += span_info("Some bushes can be used as hiding places. Toggle the 'SNEAK' button on your HUD, then click the bush to hide in it. You can stop hiding by clicking the bush again, or by moving out of it.")
		...
		if(do_after(L, SEARCHTIME, target = src))
			...
			if(prob(50) && looty.len)
				...
				if(B)
					B = new B(user.loc)
					user.put_in_hands(B)
					user.visible_message(span_notice("[user] harvests [B] from [src] bush."))
					return
