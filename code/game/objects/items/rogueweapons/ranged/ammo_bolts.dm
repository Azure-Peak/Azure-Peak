...
/obj/projectile/bullet/reusable/bolt/on_hit(atom/target)
	...
	if(can_train_combat_skill(L, /datum/skill/combat/crossbows, SKILL_LEVEL_APPRENTICE))
		L.mind.add_sleep_experience(/datum/skill/combat/crossbows, L.STAINT * skill_multiplier)
	// Anyone nearby holding a xbow can learn from the shot.
	var/shooter_skill = L.get_skill_level(/datum/skill/combat/crossbows)
	for(var/mob/living/learner in view(12, L))
		...
		if(!istype(main_hand, /obj/item/gun/ballistic/revolver/grenadelauncher/crossbow) && !istype(off_hand, /obj/item/gun/ballistic/revolver/grenadelauncher/crossbow))
			continue
		var/learner_skill = learner.get_skill_level(/datum/skill/combat/crossbows)
		...
		learner.mind.add_sleep_experience(/datum/skill/combat/crossbows, L.STAINT * skill_multiplier * 2)
