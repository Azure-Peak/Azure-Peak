...
/obj/item/mundane/puzzlebox/easy/attack_self(mob/living/user)
	...
	if(ckey in finished_ckeys)
		...
		return
	playsound(src, 'sound/items/wood_sharpen.ogg', 75, TRUE)
	playsound(src, 'sound/items/visor.ogg', 75, TRUE)
	...
	if(do_after(user,70, target = src))
		if((dice_roll) <= user.STAINT)
			...
			finished_ckeys += ckey
			playsound(src, 'sound/foley/doors/lock.ogg', 75, TRUE)
		else
			...
			finished_ckeys += ckey
			playsound(src, 'sound/foley/doors/lockrattle.ogg', 75, TRUE)
...
//		if(A.type in subtypesof(/obj/item/rogueweapon/huntingknife)) //knife
		if(istype(A, /obj/item/rogueweapon/huntingknife))
			...
			playsound(src, 'sound/items/canned_food_open.ogg', 75, TRUE)
