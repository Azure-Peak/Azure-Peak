...
/obj/structure/ritualcircle/necra/attack_hand(mob/living/user)
	...
	switch(riteselection) // put ur rite selection here
		...
		if("Shatter the Binds") // hehe do you get t. Do You Get Im So Fucking FUnny
			...
			to return to either Necra, or their corpses, potentially allowing for revival, if not peace in the afterlyfe. Place the dagger upon the \
			...
			if(!pissdagger)
				to_chat(user, span_warning("This ritual requires a Graggarite assassin's profane dagger to be placed upon the rune."))
				return

			user.visible_message("[user] places the dagger in the center of the rune, drawing spectral strands of Lux up through the air!")
			playsound(user, 'sound/vo/mobs/ghost/whisper (3).ogg', 100, FALSE, -1)
			// this may seem like a lot right now BUT this is designed to allow assassins to track their daggers down, later. potentially interrupting whatever.
			// alt. i'll amke it a requirement for them to be dead IDGAF.
			if(do_after(user, 20 SECONDS))
				playsound(user, 'sound/vo/mobs/ghost/whisper (1).ogg', 100, FALSE, -1)
				pissdagger.visible_message("[pissdagger] is suddenly thrust up into the air! Nigh-invisible threads tear at it, beginning to bend the metal!")
				if(do_after(user, 10 SECONDS))
					playsound(user, 'sound/magic/soulsteal_2.ogg', 80, TRUE)
					pissdagger.audible_message("<span style='color:#3F5C6D'>The profane dagger</span> screams, " + span_cult("<b>\"HELP US!!\"</b>"))
					if(do_after(user, 5 SECONDS))
						pissdagger.visible_message("[pissdagger] is CRUSHED and TWISTED! TERRIBLE SCREAMS sounds from within!")
						playsound(user, 'sound/vo/mobs/ghost/death.ogg', 100, FALSE, -1)
						if(do_after(user, 5 SECONDS))
