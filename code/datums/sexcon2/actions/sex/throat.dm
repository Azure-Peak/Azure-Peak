...
/datum/sex_action/sex/throat/get_finish_message(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	switch(penis.penis_type)
		if(PENIS_TYPE_BARBED, PENIS_TYPE_BARBED_KNOTTED)
			user.visible_message(span_notice("[user] rakes their barbs inside of [target]'s throat!"),
			span_notice("I rake my barbs inside of [target]'s throat."))
			target.apply_status_effect(/datum/status_effect/barbed_fellatio)
	return FALSE
