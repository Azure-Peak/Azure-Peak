...
/datum/appearance_debugger/proc/get_appearance_data(atom/appearance_owner)
	...
	var/list/data = list(
		...
		"plane" = target.plane,
		"plane_true" = target.plane, // PLANE_TO_TRUE(target.plane),
...
// Remove this if we ever get real planecube
...
#define SET_PLANE_IMPLICIT(thing, new_value) thing.plane = new_value;
...
/datum/appearance_debugger/proc/set_appearance_proxy(atom/movable/screen/map_view/proxy, appearance_id)
	...
	if(!isnull(backdrop_color))
		...
		var/mutable_appearance/backdrop = mutable_appearance('icons/effects/alphacolors.dmi', "white", SPACE_LAYER, proxy, FLOOR_PLANE, 255, KEEP_APART | RESET_COLOR | RESET_ALPHA | RESET_TRANSFORM)
