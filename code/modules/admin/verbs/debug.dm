...
/client/proc/spawn_cooking_test()
	set category = "Debug"
	set name = "Spawn Cooking Test Kit"
	if(!check_rights(R_DEBUG))
		return
	...
	new /obj/item/kitchen/spoon(T)
	new /obj/item/rogueweapon/huntingknife/cleaver(T)
	var/obj/item/reagent_containers/glass/bucket/water_bucket = new(T)
	if(water_bucket.reagents)
		water_bucket.reagents.add_reagent(/datum/reagent/water, 100)
	...
	to_chat(usr, span_notice("Spawned the cooking test kit at your location."))
	log_admin("[key_name(src)] spawned the cooking test kit.")
	SSblackbox.record_feedback("tally", "admin_verb", 1, "Spawn Cooking Test Kit")
