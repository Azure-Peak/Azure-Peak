...
/datum/job/roguetown/mercenary/after_spawn(mob/living/L, mob/M, latejoin = FALSE)
	...
	if(L && ishuman(L))
		...
		if(statue)
			...
			to_chat(M, span_notice("<a href='?src=[REF(statue)];register=[REF(H)]'>Touch the statue from afar</a> to register myself as available for contract. (This link expires in 2 minutes)"))

			// Store the registration request with a timer
			statue.pending_registrations[H.key] = H
			addtimer(CALLBACK(statue, TYPE_PROC_REF(/obj/structure/roguemachine/talkstatue/mercenary, expire_registration), H.key), 2 MINUTES)
