...
/datum/component/martyrweapon/proc/on_equip(datum/source, mob/user, slot)
	if(!allow_all)
		if(ishuman(user))
			...
			if(HAS_TRAIT(src, TRAIT_PALLID) || HAS_TRAIT(user, TRAIT_ROTMAN) || HAS_TRAIT(user, TRAIT_DEATHLESS))	//Can't be a Martyr if you're undead already.
...
/datum/job/roguetown/martyr
	...
	virtue_restrictions = list(/datum/virtue/combat/second_chance, /datum/virtue/utility/noble, /datum/virtue/utility/hollow, /datum/virtue/combat/dualwielder, /datum/virtue/heretic/zchurch_keyholder)
