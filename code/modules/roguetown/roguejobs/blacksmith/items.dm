...
/obj/item/examine(mob/user)
	...
	if(has_item_quality)
		var/qmult = ITEM_QUALITY_MULT(item_quality)
		var/qpct = round(qmult * 100)
		switch(item_quality)
			if(ITEM_QUALITY_LOOTED)
				. += span_warning("Quality: scavenged ([qpct]% value).")
			if(ITEM_QUALITY_RUINED)
				. += span_warning("Quality: ruined ([qpct]% value).")
			if(ITEM_QUALITY_AWFUL)
				. += span_warning("Quality: awful ([qpct]% value).")
			if(ITEM_QUALITY_CRUDE)
				. += span_warning("Quality: crude ([qpct]% value).")
			if(ITEM_QUALITY_ROUGH)
				. += span_info("Quality: rough ([qpct]% value).")
			if(ITEM_QUALITY_STANDARD)
				. += span_info("Quality: standard ([qpct]% value).")
			if(ITEM_QUALITY_FINE)
				. += span_info("Quality: fine ([qpct]% value).")
			if(ITEM_QUALITY_FLAWLESS)
				. += span_green("Quality: flawless ([qpct]% value).")
			if(ITEM_QUALITY_MASTERWORK)
				. += span_green("Quality: masterwork ([qpct]% value).")
