...
/obj/item/polishing_cream/attack_obj(obj/O, mob/living/user)
	...
	if(!thing.anvilrepair)
		return ..()
	if((HAS_TRAIT(user, TRAIT_SQUIRE_REPAIR) || HAS_TRAIT(user, TRAIT_SELF_SUSTENANCE) || HAS_TRAIT(user, TRAIT_BOOTSHINE) || user.get_skill_level(thing.anvilrepair)) && thing.polished == 0)
