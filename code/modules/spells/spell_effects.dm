...
/obj/effect/temp_visual/spell_impact/Initialize(mapload, impact_color = LIGHT_COLOR_WHITE, intensity = SPELL_IMPACT_LOW)
	. = ..()
	color = impact_color
	...
	switch(intensity)
		...
		if(SPELL_IMPACT_MEDIUM)
			light_outer_range = LIGHT_RANGE_FIRE + 2
		if(SPELL_IMPACT_HIGH)
			light_outer_range = LIGHT_RANGE_FIRE + 4
