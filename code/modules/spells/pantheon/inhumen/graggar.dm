...
/datum/action/cooldown/spell/graggar/graggar_battlecry/cast(atom/cast_on)
	...
	for(var/mob/living/carbon/target in view(cast_range, get_turf(owner)))
		...
		if(istype(target.patron, /datum/patron/vheslyn))
			...
			if(prob(15) && target.cmode = TRUE) //15% chance to pop a forcesay line easter egg if in combat mode
				target.say(pick("SAY NO MORE, YOU FIRST, THEN THE REST!", "LET ME SHOW YOU A REAL FUCKING SLAUGHTER!", "YOU WILL BE, THEN THEM, THEN THE REST!"), spans = list("bloody"))
			continue
