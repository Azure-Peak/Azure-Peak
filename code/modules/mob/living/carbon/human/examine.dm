			...
			if((L.STAINT > 9 && L.STAPER > 9) || HAS_TRAIT(L, TRAIT_INTELLECTUAL))
				...
				if(HAS_TRAIT(src, TRAIT_DEATHLESS) && !mind?.has_antag_datum(/datum/antagonist/vampire))
					if(stat == CONSCIOUS || stat == UNCONSCIOUS)
						...
						. += span_warning("<i>[m1] is still twitching despite [t_his] fatal condition.</i>")
				if(HAS_TRAIT(src, TRAIT_NODECAP) && stat == DEAD)
					. += span_mind_control("<i>[m1] exhudes a macabre yearning for Lux. [t_He] may be rekindled directly from it.</i>")
