/proc/get_market_pool_tier_for_category(category)
	switch(category)
		...
		if(ITEM_CAT_FOODSTUFF_FRESH, ITEM_CAT_FOODSTUFF_PRESERVED, ITEM_CAT_BEVERAGE, ITEM_CAT_GARMENT_LUXURY, ITEM_CAT_SEAFOOD)
...
/proc/get_market_theme_for_category(category)
	switch(category)
		...
		if(ITEM_CAT_FOODSTUFF_FRESH, ITEM_CAT_FOODSTUFF_PRESERVED, ITEM_CAT_BEVERAGE, ITEM_CAT_SEAFOOD)
...
/proc/get_navigator_bucket_for_category(category)
	switch(category)
		...
		if(ITEM_CAT_ARMOR_HELMETS, ITEM_CAT_ARMOR_CHESTPIECES, ITEM_CAT_ARMOR_LEGS, ITEM_CAT_ARMOR_NECK, ITEM_CAT_ARMOR_BOOTS, ITEM_CAT_ARMOR_GLOVES, ITEM_CAT_ARMOR_MASKS, ITEM_CAT_ARMOR_BRACERS, ITEM_CAT_ARMOR_BELTS, ITEM_CAT_ARMOR_BARDING, ITEM_CAT_ARMOR_LIGHT)
			return NAVIGATOR_BUCKET_ARMOR
		if(ITEM_CAT_GARMENT_COMMON, ITEM_CAT_GARMENT_FINE, ITEM_CAT_GARMENT_LUXURY)
			return NAVIGATOR_BUCKET_GARMENTS
		...
		if(ITEM_CAT_VALUABLES_RINGS, ITEM_CAT_VALUABLES_HOLY, ITEM_CAT_DECORATION, ITEM_CAT_POTTERY, ITEM_CAT_TROPHY)
			return NAVIGATOR_BUCKET_VALUABLES
		if(ITEM_CAT_BOOK_WRIT)
			return NAVIGATOR_BUCKET_BOOKS
		if(ITEM_CAT_INSTRUMENT)
			return NAVIGATOR_BUCKET_INSTRUMENTS
		if(ITEM_CAT_SEAFOOD)
			return NAVIGATOR_BUCKET_SEAFOOD
		if(ITEM_CAT_BEVERAGE)
			return NAVIGATOR_BUCKET_BEVERAGE
...
/proc/all_navigator_buckets()
	return list(
		...
		NAVIGATOR_BUCKET_ARMOR,
		NAVIGATOR_BUCKET_GARMENTS,
		NAVIGATOR_BUCKET_ENGINEERING,
		NAVIGATOR_BUCKET_VALUABLES,
		NAVIGATOR_BUCKET_BOOKS,
		NAVIGATOR_BUCKET_INSTRUMENTS,
		NAVIGATOR_BUCKET_SEAFOOD,
		NAVIGATOR_BUCKET_BEVERAGE,
...
/proc/get_navigator_bucket_pool_baseline(bucket)
	switch(bucket)
		...
		if(NAVIGATOR_BUCKET_ARMOR)
			return NAVIGATOR_BUCKET_POOL_ARMOR
		if(NAVIGATOR_BUCKET_GARMENTS)
			return NAVIGATOR_BUCKET_POOL_GARMENTS
		if(NAVIGATOR_BUCKET_ENGINEERING)
			return NAVIGATOR_BUCKET_POOL_ENGINEERING
		if(NAVIGATOR_BUCKET_VALUABLES)
			return NAVIGATOR_BUCKET_POOL_VALUABLES
		if(NAVIGATOR_BUCKET_BOOKS)
			return NAVIGATOR_BUCKET_POOL_BOOKS
		...
		if(NAVIGATOR_BUCKET_BEVERAGE)
			return NAVIGATOR_BUCKET_POOL_BEVERAGE
...
/proc/get_navigator_bucket_theme(bucket)
	switch(bucket)
		if(NAVIGATOR_BUCKET_WEAPONS, NAVIGATOR_BUCKET_ARMOR)
			return MARKET_THEME_WARGEAR
		if(NAVIGATOR_BUCKET_POTIONS_REAGENTS)
			return MARKET_THEME_ARCYNE
		if(NAVIGATOR_BUCKET_GARMENTS)
			return MARKET_THEME_GARMENTS
		if(NAVIGATOR_BUCKET_SEAFOOD, NAVIGATOR_BUCKET_BEVERAGE)
			return MARKET_THEME_FOODSTUFFS
		if(NAVIGATOR_BUCKET_VALUABLES, NAVIGATOR_BUCKET_INSTRUMENTS, NAVIGATOR_BUCKET_BOOKS)
			return MARKET_THEME_LOOT
		if(NAVIGATOR_BUCKET_ENGINEERING)
			return MARKET_THEME_ENGINEERING
	return MARKET_THEME_MISC
