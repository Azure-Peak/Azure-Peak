...
/obj/item/reagent_containers/food/snacks/rogue/noodles/attackby(obj/item/I, mob/living/user, params)
	...
	if(istype(I, /obj/item/reagent_containers/food/snacks/grown/fruit/tomato_sauce))
		if(isturf(loc)&& (found_table))
			...
			to_chat(user, span_warning("You need to put [src] on a table to FUCK this!"))
	if(istype(I, /obj/item/reagent_containers/food/snacks/rogue/pesto))
		if(isturf(loc)&& (found_table))
			playsound(get_turf(user), 'sound/foley/dropsound/food_drop.ogg', 40, TRUE, -1)
			to_chat(user, span_notice("GREEN GASTER!!!"))
			if(do_after(user,long_cooktime, target = src))
				add_sleep_experience(user, /datum/skill/craft/cooking, user.STAINT)
...
/obj/item/reagent_containers/food/snacks/rogue/spaghetti
	...
	faretype = FARE_FINE
	list_reagents = list(/datum/reagent/consumable/nutriment = SNACK_CHUNKY) //It's pasta and sauce.
